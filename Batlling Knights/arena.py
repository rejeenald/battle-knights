...
class Arena:
    ...
    def position_knight_to_arena(self, knight: Knight, from_attempted_battle:bool=False) -> None:
        if knight.status != 'DROWNED':
            ...
            if item:
                print(item)
                item.position = knight.prev_position
                self._position_item_to_arena(item)
    ...
    def _update_arena_item_status(self, loser_item):
        if loser_item == 'axe':
            self.axe.equipped = False
        elif loser_item == 'dagger':
            self.dagger.equipped = False
        elif loser_item == 'helmet':
            self.helmet.equipped = False
        else:
            self.helmet.equipped = False
